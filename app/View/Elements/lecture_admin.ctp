...
<div class="lecture">
	<div class="lecture-title">
		...
		foreach ($Lecture['Studentlearnlecture'] as $key => $student) {
			$listStudent[$key] = $student['userid'];
		}

		if($this->Session->read('role') == 'student'){
			if(in_array($this->Session->read('userid'),$listStudent)){
				echo $this->Html->link($Lecture['Lecture']['title'],array('controller' => "lectures", "action"=>"learn",$Lecture['Lecture']['lectureid'] ),array("escape"=>false,'confirm'=>false));
			}else{
	...
	<?php if($this->Session->read('role') == 'teacher'){  
		...
		<div class="lecture-button2">
			...
			echo $this->Html->link($this->Form->button('登録',array('class' => "btn btn-success btn-sm")),array('controller' => "students", "action"=>"studentlearnlecture",$Lecture['Lecture']['lectureid']),array("escape"=>false,'confirm'=>'Are you sure?'));?>
		...
		<div class="lecture-vote">
			...
			if($Lecture)
				echo $this->Html->div(array("class"=>"like"),$this->Html->link($this->Html->image('like.png'),array('controller' => 'lectures','action' => 'like', $Lecture['Lecture']['lectureid']),
					array('escape' =>false )));
			//pr($Lecture);die();
			if($Lecture['User']['role']=='teacher')
			{
				echo $this->Html->para(null,count($Lecture['Like'])."いいね",array("class"=>"p"));
			}
			if($Lecture['User']['role']=='student')
			{
	...
	<div class="lecture-author">
		...
		if($Lecture['User']['role']=='teacher')
		{
			echo $this->Html->div(array("class"=>"author"),$this->Html->link($Lecture['User']['username'],array('controller' => "users", "action"=>"user_view_teacher",$Lecture['User']['userid']),
				array("escape"=>false)));
		}
		if($Lecture['User']['role']=='student')
		{
