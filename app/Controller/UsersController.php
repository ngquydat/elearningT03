	...
	class UsersController extends AppController{
		...
		public function login(){
			...
			//$this->Session->setFlash('現在のIPアドレス前回のIPアドレスではない','default',array('class' => 'success1'));
				...
				if (in_array($_SERVER['REMOTE_ADDR'], $ipAdmin)) {
					if($this->Auth->login()){
						...
						if($this->Adminip->findByUseridAndIpid($this->Session->read('userid'),$_SERVER['REMOTE_ADDR'])){
							...
							$this->Session->setFlash('現在のIPアドレス前回のIPアドレスではない又はあなたは管理者ではない!','default',array('class' => 'warning1'));
						}

					}else{
						$this->Session->setFlash('ユーザ名とパスワードは不正です','default',array('class' => 'error1'));
					}
						...
						if($this->Auth->login()){
							...
							if($this->Session->read('role') == 'admin'){
								if($this->Adminip->findByUseridAndIpid($this->Session->read('userid'),$_SERVER['REMOTE_ADDR'])){
									...
									$this->Auth->logout();
									$this->Session->destroy();
									$this->Session->setFlash('現在のIPアドレス前回のIPアドレスではない','default',array('class' => 'error1'));
								}
							...
							if($this->Session->read('role') == 'teacher'){
								if($this->Session->read('ipchecked')==-1){
									$this->Session->setFlash('Verifycodeをもう一度入力してください','default',array('class' => 'notice1'));
					...
					{
						$this->Session->write('ipchecked',-1);
						$this->Session->setFlash('ブロックした！','default',array('class' => 'error1'));
					}
				}
			}
		}
		...
		public function register()
		{
			...
			if($this->request->is("post")){
				...
				if(strcmp($this->request->data['Registerrequest']['password'],$this->request->data['Registerrequest']['password2'])!=0){
					$this->Session->setFlash('パスワード入力は不正である','default',array('class' => 'error1'));
					return;
				}
				...
				if(!$this->Registerrequest->save($this->data)){
					...
					$this->Session->setFlash("管理者に登録情報を送りましたので、管理者が確認するまでにお待ちください",'default',array('class' => 'success1'));
			...
			public function change_profile($id=null){
				...
				if($this->request->is('post')){
					...
					if($this->User->saveAll($this->request->data)){
						$this->Session->setFlash('プロフィール情報を格納しました','default',array('class' => 'success1'));
					}
				...
				if(!strcmp(AuthComponent::password($this->data['User']['oldpassword']),$user['User']['password']))
				{
					if(!strcmp($this->data['User']['newpassword1'],$this->data['User']['newpassword'])) {
						...
						if($this->User->save($data)){
							$this->Session->setFlash('パスワードを変更しました','default',array('class' => 'success1'));
							return $this->redirect($this->referer());
						}
						$this->Session->setFlash('パスワードを変更できません','default',array('class' => 'error1'));
						return $this->redirect($this->referer());
					}
			...
			if($this->request->is('post')){
					...
					!strcmp(AuthComponent::password($this->data['User']['oldverifycode']),$user['User']['verifycode']))
				{
					...
					$data =  array(
						...
						// debug($data);die();
					if($this->User->save($data)){
						$this->Session->setFlash('verifycode変更しました','default',array('class' => 'success1'));
						return $this->redirect($this->referer());
					}
					$this->Session->setFlash('verifycode変更できませんでした','default',array('class' => 'error1'));
					return $this->redirect($this->referer());
				} 
			}
			if (!$this->request->data) {
			...
			if($this->request->is('post')){
				...
				if(!strcmp (AuthComponent::password($this->request->data['User']['mypassword']), $user['User']['password'])) {
					...
					if (!strcmp (AuthComponent::password($this->request->data['User']['answer']), $user['User']['verifycode'])) {
						...
						$this->Auth->logout();
						$this->Session->setFlash("あなたのアカウントを削除しました",'default',array('class' => 'success1'));
						$this->Session->destroy();
						return $this->redirect($this->Auth->redirect());
					}
				} 
		...
		public function reset_password($id = null) // Sua lai database
		{
			...
			if($this->User->updateAll(
				...
				$this->Session->setFlash("リセットしました",'default',array('class' => 'success1'));
			return $this->redirect($this->referer());
		}
		$this->Session->setFlash("resetできませんでした",'default',array('class' => 'error1'));
	...
	public function accept_register($id=null)
	{
		...
		if(count($registerrequests)==0) {
			$this->Session->setFlash('登録リクエストはないです','default',array('class' => 'notice1'));
			return;
		} 
	...
	public function admin_view_user($id=null)
	{
		...
		if($this->request->is('post')){
			...
			$this->Session->setFlash('プロフィール情報を格納することができませんでした！','default',array('class' => 'error1'));
		}
		if(!$this->request->data){
			$this->request->data = $user;
			$this->set('user',$user);
	...
	public function user_view_student($id=null)
	{
		...
		$this->set('user',$user);
		$this->paginate = array(
			'conditions' => array('Studentlearnlecture.userid' => $id),
			'limit' => 2,
			'recursive' => 3
			);
		$this->set('Lectures', $this->paginate('Studentlearnlecture'));
		//pr( $this->paginate('Studentlearnlecture')); die();
	}
	...
	public function add_ip()
	{
		...
		if($this->request->is('post')){
			...
			if($this->Adminip->find('count',array('conditions'=>array('userid'=>$data['Adminip']['userid'],'ipid'=>$data['Adminip']['ipid']))))
				$this->Session->setFlash("IP: ".$data['Adminip']['ipid']." は存在しています",'default',array('class' => 'error1'));
			else
	...
	public function check_verifycode(){
		...
		if($this->request->is('post')){
			...
			if(($user['User']['secretquestion']==(AuthComponent::password($data['User']['secretquestion'])))&&($user['User']['verifycode']==(AuthComponent::password($data['User']['answer']))))
			{
				...
				$this->Session->setFlash('質問と答えは不正で、再入力してください','default',array('class' => 'error1'));
			}
		}
	}
	public function report(){
		if($this->request->is('post')){
			...
			$this->Session->setFlash("違反報告はしました",'default',array('class' => 'success1'));
			return $this->redirect($this->referer());
		}
		return $this->redirect($this->referer());
	}
